...
事件捕获：js:addEventListener('click', fn, true);react:onClickCapture = fn
事件冒泡
```
...
新生命周期：
    ...
    getSnapshotBeforeUpdate:可以获取更新前的props、state，返回更新前的参数，传给componentDidUpdate。
...
开启BFC:
	1.设置float:left(不推荐)
	2.display:inline-block、table-cell、flex、table-caption或者inline-flex(不推荐)
    ...
    cors分类：
        ...
        简单请求：
        	满足两大条件的请求就是简单请求
				...
				(3)不能为XMLHttpRequestUpdate注册监听器
				(4)请求中没有使用readableStream对象
		非简单请求：
        	只要不满足上边任意一条就属于非简单请求。
            每次发送前都得发送预检请求，当前页面是否支持跨域，支持哪些请求方式，当前网页所在的域名是否在服务器的许可名单之中，以及可以使用哪些http动词和头信息字段，只有得到肯定答复才会正式发出请求，否则就报错。
...
```html
概念：同名函数根据参数类型、参数个数来区分同名函数。
js没有函数重载，但是可以根据arguments的个数来区分调用方法。
```
...
```html
当setState进入react调度的话，就变成异步任务了。
当未进入react调度，比如使用setTimeout、setInterval，就是同步任务。

如果setState进入“异步”过程，连续setState5次，只取最后一次结果（批量更新机制）。
如果setState进入“同步”过程，连续setState5次，都会记录，异步过程里有批量更新的操作。

合成事件（比如点击事件）：
	- 会经历try finally，除修改state操作的其他操作在try中进行，而真正修改state是在finally中进行的。所以是异步
生命周期：
	- 和合成事件类似，异步
原生事件：
	- 原生事件不会被return到其他步骤，直接到修改state的步骤，所以是同步
setTimeout：
	- 可以存在在任何情况下，（比如在合成事件中。因为在执行try里的操作时，遇到setTimeout放入到宏任务队列，接着进行finally，finally执行完state也已经改完了，再执行setTimeout时已经是更新后的结果，所以是同步的）
```
...
## 129. 一个文件请求到服务器CDN的过程

```html
如果没有cdn的话，和往常的做法一样。
1.如果在使用cdn的情况下，DNS域名解析的结果实际是一个CNAME，该CNAME通过DNS解析指向一个全局负载均衡设备的ip地址，将该ip地址返回到客户端。
2.用户向该全局负载均衡设备发送url请求，全局负载均衡根据用户IP地址和用户请求的url，选择一个该区域的一台区域负载均衡设备。
3.区域负载均衡通过判断找出距离客户距离较近且拥有用户所需信息的缓存服务器，将该缓存服务器的ip地址返回到全局负载均衡，全局负载均衡将ip地址返回到用户。
4.用户向该缓存服务器发送请求，如果该服务器没有该内容，将会找到上一级缓存服务器直到源本地服务器，返回信息
```
...
## 133. span设置为float：left或position：absolute

```html
span可以设置宽高，因为变为inline-block
```

## 134. 获取真实像素

```html
pc端：window.screen.width可以直接获取物理像素。
手机端：无法直接获取到像素。通过css像素*（真实像素和css像素比值）获取真实像素
css像素：window.screen.width或者document.documentElement.clientWidth
像素比值：window.devicePixelRatio
...
```html
因为js是通过IEEE754双精度浮点数（1位符号位，11位阶数，52位尾数）来存储小数的。0.1首先转换为二进制数0.00011001100...，然后转为双精度浮点数。为1.1001100...*2^-4。0.2转换为二进制数为0.0011001100...，然后转为1.1001100*2^-3。让这两数相加，因为最后要保留52位尾数，再转为十进制数，所以结果为0.3000000004不等于0.3
```

## 136. jquery的获取元素方法

```html
通过判断选择器里的第一个元素来判断获取方法。如果为#使用getElementById，如果使用.使用getElementsByClassName，如果使用其他的使用getElementsByTagName。
```

## 137. innerText和innerHtml的区别

```html
innerText获取到和修改的是文本内容，不识别标签
innerHtml获取和修改的是文本内容，识别标签。
```
...
1. 我们假定有一个请求`request`与一个限制最多同时`3`个请求的调度器，要求实现一次最多处理`limit`个数的请求：
...
2. 实现一个限制并发的异步调度器`Scheduler`，保证同时运行的任务最多2个，完善如下代码，使程序能正常输出：
...
3. 题三同样是限制器问题，不过这次参数是直接传入了一个数组，要求同样是是并发最多同时处理3个请求，但要求如果全部成功则返回结果的数组，且结果顺序与参数顺序保持一致，如果失败则直接返回失败，是不是有点`Promise.all()`的意思了：
...
块元素：
    ...
    3.auto默认为父元素的100%
行内元素：（span、a）
	1.不可以设置宽高、margin和padding的上下。（padding的上下设置会产生显示，但是会有渲染bug）
    2.宽高默认为内容的长度和高度
    3.每个行内元素可以在一行，如果编辑时是换行的，那么在页面显示会有间隙。需要将它的上一级改为font-size=0
	4.可以设置行高，等同于给父元素设置行高
    5.行内元素中不可以放置块元素（其实也可以，块元素会另起一行）
行内块元素：（input、img）
	1.可以设置任何元素
    2.每个行内块元素可以在一行，如果编辑时是换行的，那么在页面显示会有间隙。需要将它的上一级改为font-size=0
    3.默认是内容的宽高
```
...
```html
name：
	name和content一起使用，name为键，content为值
	-author:表示作者名字
	-description：描述页面
	-keywords：与页面相关的关键词
	-viewpoint：视口大小
	-robots：爬虫对此页面的处理行为
	-renderer：对双核浏览器的渲染方式
http-equiv:
	http-equiv和content一起使用
	- X-UA-Compatible：做浏览器适配
	- content-type：声明文档类型和字符集
	-x-dns-prefetch-control：对https协议的网站打开dns对a标签的提前解析
```
